    ...
    if (method === "GET") {
        if (path.startsWith("/jokes")) {
            ...
            if (num1 && num2 && operacion) {
                let auxiliar: number;
                if (operacion === "suma") {
                    auxiliar = parseInt(num1) + parseInt(num2);
                    ...
                    if (num2 === "0") {
                        return new Response("No se puede dividir por 0.", {
                            status: 400,
                        });
                    }
                    auxiliar = parseInt(num1) / parseInt(num2);
                } else {
                    return new Response("error, vuelva a intentarlo");
                }
                const resultado = auxiliar;
                return new Response(
                    `El resultado de la operacion: ${operacion} es: ${resultado}`,
                );
            }
